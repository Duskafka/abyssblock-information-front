...
export default function ModGuideModal({ isOpen, onClose }: ModGuideModalProps) {
    ...
    return (
        <Modal
            open={isOpen}
            onClose={onClose}
            title={<><span className="text-lg">🌀</span> Abyssblock 모드 토탈 가이드</>}
            size="lg"
        >
            <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-abyss-900 border border-slate-800 p-4 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            <div>
                                <span className="text-xs font-bold text-slate-200 block">Abyssblock 전용 파일 (gold-crop-obf.jar)</span>
                                <p className="text-2xs text-slate-500 font-mono mt-0.5">v2.0.0 / 실행용 전용 모드 파일</p>
                            </div>
                            <a
                        ...
                        <div className="text-2xs bg-rose-500/5 text-slate-400 border border-rose-500/10 p-2.5 rounded-lg leading-relaxed">
                            <span className="text-rose-400 font-bold">⚠️ 알림:</span> 파일이 `.jar` 확장자이므로 크롬 등 브라우저에서 '안전하지 않은 파일' 경고가 발생할 수 있습니다. 수동으로 암호화 처리된 안전한 파일이오니 안심하고 <span className="text-emerald-400 font-bold">'유지'</span> 또는 <span className="text-emerald-400 font-bold">'계속 다운로드'</span>를 눌러 구동해 주세요.
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {commands.map((cmd) => (
                                <div key={cmd.id} className="bg-abyss-900 border border-slate-800 p-4 rounded-xl space-y-2 relative group">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button type="button"
                                            ...
                                            className="text-2xs bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200 border border-slate-700 font-mono px-2 py-0.5 rounded transition"
                                        >
                                            {copiedText === cmd.id ? '복사완료! ✓' : 'COPY'}
                                        </button>
                                    </div>

                                    <div className="w-full bg-slate-950/70 border border-slate-800 font-mono px-3 py-2 text-xs text-emerald-400 font-bold flex justify-between items-center rounded-lg">
                                        <span>{cmd.syntax}</span>
                                    </div>

                                    <p className="text-sm text-slate-300 leading-relaxed pl-0.5">{cmd.description}</p>

                                    <div className="text-2xs bg-amber-400/5 text-slate-400 border border-amber-400/10 p-2.5 rounded-lg leading-relaxed">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="bg-abyss-900 border border-slate-800 p-4 rounded-xl flex flex-col justify-center">
                                <span className="text-2xs text-slate-500 font-bold mb-1">인게임 기본 결제</span>
                            ...
                            <div className="bg-abyss-900 border border-amber-400/10 p-4 rounded-xl relative overflow-hidden flex flex-col justify-center">
                                <div className="absolute top-0 right-0 bg-rose-500/20 text-rose-400 text-2xs font-black px-2 py-0.5 rounded-bl">강력 추천!</div>
                                <span className="text-2xs text-amber-400/80 font-bold mb-1">대리 후원 특가</span>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                            <div className="p-3.5 bg-abyss-900 border border-slate-800 rounded-xl space-y-1">
            ...
            <div className="mt-6 border-t border-slate-800 pt-4 text-center">
                <button
                    type="button"
                    onClick={onClose}
                    className="w-full sm:w-auto bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs px-6 py-2 rounded-xl border border-slate-700 transition focus-ring"
                >
                    닫기
                </button>
            </div>
